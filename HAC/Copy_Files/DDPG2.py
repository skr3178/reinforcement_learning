...
class DDPG:
    ...
    def load(self, directory, name):
        try:
            ...
            try:
                # If that fails, try loading as TorchScript model
                self.actor = torch.jit.load('%s/%s_actor.pth' % (directory, name), map_location='cpu')
                self.critic = torch.jit.load('%s/%s_critic.pth' % (directory, name), map_location='cpu')
